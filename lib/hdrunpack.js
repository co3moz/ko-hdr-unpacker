...
module.exports = async function hdrunpack(hdrFile) {
  ...
  let srcFile = path.resolve(path.dirname(hdrFile), path.basename(hdrFile, '.hdr') + '.src');
  let start = Date.now();
  ...
  if (total > 16000) throw new Error('this file does not look like hdr! record count should be less than 16000 and it is ' + total + ' (' + hdrFile + ')');

  let files = [];
  for (let i = 0; i < total; i++) {
    let nameLength = await readInteger(fd);
    if (nameLength > 2048) throw new Error('this file does not look like hdr! name length should be less than 2048 and it is ' + nameLength + ' (' + hdrFile + ')');
  ...
  console.log('all files are unpacked! (' + (Date.now() - start) +' ms) total: ' + total);

  if (stat.size - size > 0) {
    console.log('WARNING: Source file has ' + sizeToText(stat.size - size) + ' unnecessary data.');
  }
}
...
function sizeToText(n) {
  ...
  if (n < 1024 * 1024 * 1024) return (parseInt(n / 1024 / 1024 * 100) / 100) + ' mb';
